namespace SagaMap.Skill.SkillDefinations.Event
{
    ...
    public class MAG_INT_DEX_UP : ISkill
    {
        ...
        (short) 10
            ...
            skill.Variable.Add("MAG_INT_DEX_UP_DEX", (int)numArray1[num - 1]);
            actor.Status.dex_skill += numArray1[num - 1];
            if (skill.Variable.ContainsKey("MAG_INT_DEX_UP_INT"))
                skill.Variable.Remove("MAG_INT_DEX_UP_INT");
            skill.Variable.Add("MAG_INT_DEX_UP_INT", (int)numArray2[num - 1]);
            actor.Status.int_skill += numArray2[num - 1];
            if (skill.Variable.ContainsKey("MAG_INT_DEX_UP_MAG"))
                skill.Variable.Remove("MAG_INT_DEX_UP_MAG");
            skill.Variable.Add("MAG_INT_DEX_UP_MAG", (int)numArray3[num - 1]);
            actor.Status.mag_skill += numArray3[num - 1];
