namespace SagaMap.Skill.SkillDefinations.Monster
{
    ...
    /// <summary>
    /// Defines the <see cref="MobHolyfeather" />.
    /// </summary>
    public class MobHolyfeather : ISkill
    {
        ...
        {
            int lifetime = 12000;
            foreach (SagaDB.Actor.Actor actor in Singleton<MapManager>.Instance.GetMap(sActor.MapID).GetActorsArea(sActor, (short)200, true))
            {
                if (actor.type == ActorType.MOB)
                {
                    DefaultBuff defaultBuff = new DefaultBuff(args.skill, actor, nameof(MobHolyfeather), lifetime, 3000);
