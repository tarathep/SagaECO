namespace SagaMap.Skill.SkillDefinations.Marionest
{
    ...
    /// <summary>
    /// Defines the <see cref="MarioEarthWater" />.
    /// </summary>
    public class MarioEarthWater : ISkill
    {
        ...
        {
            if (((ActorPC)sActor).Marionette == null)
                return;
            float MATKBonus = (float)(2.09999990463257 + 0.300000011920929 * (double)level);
            List<SagaDB.Actor.Actor> actorsArea = Singleton<MapManager>.Instance.GetMap(sActor.MapID).GetActorsArea(sActor, (short)400, false);
