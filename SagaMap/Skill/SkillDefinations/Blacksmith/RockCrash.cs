namespace SagaMap.Skill.SkillDefinations.Blacksmith
{
    ...
    /// <summary>
    /// Defines the <see cref="RockCrash" />.
    /// </summary>
    public class RockCrash : ISkill
    {
        ...
        {
            float ATKBonus = 1f;
            if (dActor is ActorMob && ((ActorMob)dActor).BaseData.mobType.ToString().ToLower().IndexOf("rock") > -1)
                ATKBonus += 1f * (float)level;
            ...
            if (!Singleton<SkillHandler>.Instance.CanAdditionApply(sActor, dActor, SkillHandler.DefaultAdditions.硬直, rate))
                return;
            硬直 硬直 = new 硬直(args.skill, dActor, 5000);
