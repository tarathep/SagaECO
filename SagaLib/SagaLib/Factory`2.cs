namespace SagaLib
{
    ...
    /// <summary>
    /// Defines the <see cref="Factory{K, T}" />.
    /// </summary>
    ...
    public abstract class Factory<K, T> where K : new() where T : new()
    {
        ...
        private void ParseNode(XmlElement ele, T item)
        {
            foreach (object childNode in ele.ChildNodes)
            {
                if (childNode.GetType() == typeof(XmlElement))
                {
                    ...
                    {
                        Logger.ShowError("Error on parsing:" + this.path);
                ...
                foreach (object obj1 in childNodes)
                {
                    ...
                    if (obj1.GetType() == typeof(XmlElement))
                    {
                        ...
                        catch (Exception ex)
                        {
                            ...
                            Logger.ShowError(this.GetKey(obj2).ToString());
                            Logger.ShowError(ex);
                        }
        ...
        public static K Instance
        {
            get
            {
                return Factory<K, T>.SingletonHolder.instance;
            }
            set
            {
                Factory<K, T>.SingletonHolder.instance = value;
            }
        }
