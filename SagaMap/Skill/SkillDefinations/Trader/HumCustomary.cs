namespace SagaMap.Skill.SkillDefinations.Trader
{
    ...
    /// <summary>
    /// Defines the <see cref="HumCustomary" />.
    /// </summary>
    public class HumCustomary : ISkill
    {
        ...
        public void Proc(SagaDB.Actor.Actor sActor, SagaDB.Actor.Actor dActor, SkillArg args, byte level)
        {
            ...
            if (pet == null || !Singleton<SkillHandler>.Instance.CheckMobType((ActorMob)pet, "HUMAN"))
                return;
            uint num = 30U * (uint)level;
            ActorPC actorPc = (ActorPC)sActor;
            if ((long)actorPc.Gold >= (long)num)
            {
                actorPc.Gold -= (int)num;
                Singleton<SkillHandler>.Instance.GetMobAI(pet).CastSkill(6401U, level, dActor);
            }
        }
    }
}
