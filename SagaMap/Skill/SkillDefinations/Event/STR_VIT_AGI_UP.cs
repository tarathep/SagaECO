namespace SagaMap.Skill.SkillDefinations.Event
{
    ...
    public class STR_VIT_AGI_UP : ISkill
    {
        ...
        (short) 15
            ...
            skill.Variable.Add("STR_VIT_AGI_UP_STR", (int)numArray1[num - 1]);
            actor.Status.str_skill += numArray1[num - 1];
            if (skill.Variable.ContainsKey("STR_VIT_AGI_UP_VIT"))
                skill.Variable.Remove("STR_VIT_AGI_UP_VIT");
            skill.Variable.Add("STR_VIT_AGI_UP_VIT", (int)numArray2[num - 1]);
            actor.Status.vit_skill += numArray2[num - 1];
            if (skill.Variable.ContainsKey("STR_VIT_AGI_UP_AGI"))
                skill.Variable.Remove("STR_VIT_AGI_UP_AGI");
            skill.Variable.Add("STR_VIT_AGI_UP_AGI", (int)numArray3[num - 1]);
            actor.Status.agi_skill += numArray3[num - 1];
