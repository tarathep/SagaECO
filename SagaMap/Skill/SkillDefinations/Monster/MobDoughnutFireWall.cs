namespace SagaMap.Skill.SkillDefinations.Monster
{
    ...
    /// <summary>
    /// Defines the <see cref="MobDoughnutFireWall" />.
    /// </summary>
    public class MobDoughnutFireWall : ISkill
    {
        ...
        public void Proc(SagaDB.Actor.Actor sActor, SagaDB.Actor.Actor dActor, SkillArg args, byte level)
        {
            ...
            new MobDoughnutFireWall.Activator(sActor, _dActor, args, level).Activate();
        }
        ...
        private class Activator : MultiRunTask
        {
            ...
            public override void CallBack(object o)
            {
                ...
                try
                {
                    if (this.lifetime > 0)
                    {
                        List<SagaDB.Actor.Actor> actorsArea = this.map.GetActorsArea((SagaDB.Actor.Actor)this.actor, (short)100, false);
                        List<SagaDB.Actor.Actor> dActor1 = new List<SagaDB.Actor.Actor>();
                        foreach (SagaDB.Actor.Actor dActor2 in actorsArea)
                        {
                            if (Singleton<SkillHandler>.Instance.CheckValidAttackTarget(this.sActor, dActor2) && (int)dActor2.X != (int)this.actor.X && (int)dActor2.Y != (int)this.actor.Y)
                                dActor1.Add(dActor2);
                        }
                        if (dActor1.Count > 0)
                        {
                            float MATKBonus = 2.1f;
                            Singleton<SkillHandler>.Instance.MagicAttack(this.sActor, dActor1, this.skill, Elements.Fire, MATKBonus);
                        }
