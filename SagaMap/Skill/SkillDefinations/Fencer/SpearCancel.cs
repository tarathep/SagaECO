namespace SagaMap.Skill.SkillDefinations.Fencer
{
    ...
    /// <summary>
    /// Defines the <see cref="SpearCancel" />.
    /// </summary>
    public class SpearCancel : ISkill
    {
        ...
        private bool CheckPossible(SagaDB.Actor.Actor sActor)
        {
            ...
            return actorPc.Inventory.Equipments.ContainsKey(EnumEquipSlot.RIGHT_HAND) && (actorPc.Inventory.Equipments[EnumEquipSlot.RIGHT_HAND].BaseData.itemType == ItemType.SPEAR || actorPc.Inventory.GetContainer(ContainerType.RIGHT_HAND2).Count > 0);
        }
        ...
        public void Proc(SagaDB.Actor.Actor sActor, SagaDB.Actor.Actor dActor, SkillArg args, byte level)
        {
            ...
            if (!this.CheckPossible(possesionedActor))
                return;
            int lifetime = 20000;
            DefaultBuff defaultBuff = new DefaultBuff(args.skill, possesionedActor, nameof(SpearCancel), lifetime);
        ...
        {
            actor.Status.aspd_skill_perc += (float)(1.20000004768372 + 0.3 * (double)skill.skill.Level);
        ...
        {
            float num = (float)(1.20000004768372 + 0.3 * (double)skill.skill.Level);
